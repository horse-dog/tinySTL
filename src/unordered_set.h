...
template<class, class, class, class> friend class unordered_multiset;
 ...
 public:
  ...
  void merge(unordered_set<_Key, _Hash, _Pred2, _Alloc>& __source) 
  ...
  template <class _Pred2>
  void merge(unordered_multiset<_Key, _Hash, _Pred2, _Alloc>& __source) 
  { return merge(tinySTL::move(__source)); }
...
template<class, class, class, class> friend class unordered_multiset;
 ...
 public:
  ...
  template <InputIterator Iterator>
  unordered_multiset(Iterator __first, Iterator __last)
  : _M_ht(0, hasher(), key_equal(), allocator_type()) 
  { insert(__first, __last); }
  ...
  unordered_multiset& operator=(std::initializer_list<value_type> __l) 
  {
    clear();
    insert(__l);
    return *this;
  ...
  void swap(unordered_multiset& __u) { _M_ht.swap(__u._M_ht); }

  friend bool operator==(const unordered_multiset& __x, const unordered_multiset& __y) 
  {
    if (&__x != &__y) {
      return __x.size() == __y.size() 
        && __x.bucket_count() == __y.bucket_count()
